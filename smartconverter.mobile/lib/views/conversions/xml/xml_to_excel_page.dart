...
class _XmlToExcelPageState extends State<XmlToExcelPage> with AdHelper<XmlToExcelPage> {
  ...
  Future<void> _convertXmlToExcel() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'XML-to-Excel');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting XML to Excel...';
    });
    ...
    try {
      ...
      final result = await _service.convertXmlToExcel(
        _selectedFile!,
  ...
  Future<void> _saveExcelFile() async {
    ...
    try {
      final directory = await FileManager.getXmlToExcelDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert XML data to Excel spreadsheets.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertXmlToExcel : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
