...
class FileFormatterCommonPage extends StatefulWidget {
  ...
  final String outputExtension; // pass empty string if variable or same as input
  final String apiEndpoint;
  final String outputFolder;
  final bool isVariableOutput; 
...
class _FileFormatterCommonPageState extends State<FileFormatterCommonPage> with AdHelper {
  ...
  Future<void> _pickFile() async {
    try {
      List<String> allowedExtensions = widget.inputExtension == 'all' 
          ? ['json', 'xml', 'xsd'] // Generic allow list
  ...
  Future<void> _saveFile() async {
    ...
    try {
      ...
      final formatterRoot = Directory('${root.path}/FileFormatter');
      if (!await formatterRoot.exists()) {
        await formatterRoot.create(recursive: true);
      }
      final targetDir = Directory('${formatterRoot.path}/${widget.outputFolder}');
  ...
  void _updateSuggestedFileName() {
    ...
    // Default suggestion suffix based on tool
    String suffix = '_processed';
    if (widget.toolName.toLowerCase().contains('format')) suffix = '_formatted';
    if (widget.toolName.toLowerCase().contains('minify')) suffix = '_minified';

    final baseName = p.basenameWithoutExtension(_selectedFile!.path);
    final sanitized = _sanitizeBaseName(baseName + suffix);
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Format, Validate, and Optimize code files.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildSelectedFileCard() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.description, color: AppColors.primaryBlue, size: 24),
          const SizedBox(width: 12),
          Expanded(
            child: Text(
  ...
  Widget _buildFileNameField(String helperText) {
    ...
    final hintText = _suggestedBaseName ?? 'processed_file';
