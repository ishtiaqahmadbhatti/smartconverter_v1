...
class PdfToWordOfficePage extends StatefulWidget {
  const PdfToWordOfficePage({super.key});

  @override
  State<PdfToWordOfficePage> createState() => _PdfToWordOfficePageState();
}

class _PdfToWordOfficePageState extends State<PdfToWordOfficePage> {
  ...
  Future<void> _convertPdfToWord() async {
    ...
    try {
      ...
      final result = await _service.convertPdfToWord(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'Word document ready!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Word document ready: ${result.fileName}'),
  ...
  Future<void> _saveWordFile() async {
  ...
  Future<void> _shareWordFile() async {
    ...
    if (!await fileToShare.exists()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Word document is not available on disk.'),
            backgroundColor: AppColors.error,
          ),
        );
      }
    ...
    ], text: 'Converted Word document: ${result.fileName}');
  }
  ...
  String _ensureDocxExtension(String base) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Turn PDFs into fully editable Word documents with preserved headings, paragraphs, and tables.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        helperText: '.docx extension is added automatically',
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertPdfToWord : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
          ...
          Row(
            children: [
              Flexible(
                flex: isSaved ? 3 : 1,
                child: ElevatedButton.icon(
                  onPressed: _isSaving ? null : _saveWordFile,
                  icon: _isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '2',
            'Optionally set a custom Word filename.',
          ),
          const SizedBox(height: 8),
          _buildInstructionStep('3', 'Tap "Convert to Word" and wait.'),
          const SizedBox(height: 8),
          _buildInstructionStep('4', 'Save the .docx file locally or share it.'),
        ],
      ),
    );
  }
