...
class _BsonToCsvPageState extends State<BsonToCsvPage> with AdHelper {
  ...
  String _statusMessage = 'Select a BSON file to begin.';
  ...
  Future<void> _pickBsonFile() async {
    try {
      ...
      if (extension != '.bson') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select a BSON file.',
          );
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Only BSON files are supported.'),
      ...
      setState(() {
        ...
        _savedFilePath = null;
        _statusMessage = 'BSON file selected: ${p.basename(file.path)}';
      ...
      final message = 'Failed to select BSON file: $e';
      if (mounted) {
  ...
  Future<void> _convertBsonToCsv() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a BSON file first.'),
          backgroundColor: AppColors.warning,
        ),
      );
      return;
    ...
    final adWatched = await showRewardedAdGate(toolName: 'BSON to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting BSON to CSV...';
    });
    ...
    try {
      ...
      final result = await _service.convertBsonToCsv(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'BSON converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getBsonToCsvDirectory();
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    if (base.toLowerCase().endsWith('.bson')) {
  ...
  void _resetForNewConversion() {
    setState(() {
      ...
      _suggestedBaseName = null;
      _savedFilePath = null;
      _statusMessage = 'Select a BSON file to begin.';
      _fileNameController.clear();
      resetAdStatus(null);
    });
  }
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform BSON data into CSV format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickBsonFile,
            icon: const Icon(Icons.file_open_outlined),
            label: Text(
              _selectedFile == null ? 'Select BSON File' : 'Change File',
            ),
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_bson';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertBsonToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
