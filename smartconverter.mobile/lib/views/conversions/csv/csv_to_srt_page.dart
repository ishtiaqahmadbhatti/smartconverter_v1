...
class CsvToSrtPage extends StatefulWidget {
  const CsvToSrtPage({super.key});

  @override
  State<CsvToSrtPage> createState() => _CsvToSrtPageState();
}

class _CsvToSrtPageState extends State<CsvToSrtPage> {
  ...
  Future<void> _convertCsvToSrt() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        setState(() {
          ...
          _statusMessage = 'CSV converted to SRT successfully!';
        });

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('SRT file ready: $outputFilename'),
  ...
  Future<void> _saveSrtFile() async {
    ...
    try {
      final targetDir = await FileManager.getCsvToSrtDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform CSV data into SRT subtitle format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertCsvToSrt : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
