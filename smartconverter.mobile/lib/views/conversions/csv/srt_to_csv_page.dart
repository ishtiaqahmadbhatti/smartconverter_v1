...
class SrtToCsvPage extends StatefulWidget {
  ...
  @override
  State<SrtToCsvPage> createState() => _SrtToCsvPageState();
}

class _SrtToCsvPageState extends State<SrtToCsvPage> {
  ...
  Future<void> _pickSrtFile() async {
    try {
      ...
      if (extension != '.srt') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select an SRT file.',
          );
      ...
      setState(() {
        ...
        _downloadUrl = null;
        _savedFilePath = null;
        _statusMessage = 'SRT file selected: ${p.basename(file.path)}';
      });
  ...
  Future<void> _convertSrtToCsv() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting SRT to CSV...';
      _convertedFile = null;
      _downloadUrl = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        setState(() {
          ...
          _statusMessage = 'SRT converted to CSV successfully!';
        });
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getSrtToCsvDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform SRT subtitles into CSV format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_srt';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertSrtToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
