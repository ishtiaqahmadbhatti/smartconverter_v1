...
class _CsvToJsonPageState extends State<CsvToJsonPage> with AdHelper {
  ...
  Future<void> _convertCsvToJson() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'CSV to JSON');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      setState(() {
        ...
        _statusMessage = 'CSV converted to JSON successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveJsonFile() async {
    ...
    try {
      final targetDir = await FileManager.getCsvToJsonDirectory();
