...
class _HtmlTableToCsvPageState extends State<HtmlTableToCsvPage> with AdHelper {
  ...
  Future<void> _pickHtmlFile() async {
    try {
      ...
      if (extension != '.html' && extension != '.htm') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select an HTML file.',
          );
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Only HTML files are supported.'),
  ...
  Future<void> _convertHtmlToCsv() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'HTML to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting HTML Table to CSV...';
    });
    ...
    try {
      ...
      setState(() {
        ...
        _statusMessage = 'HTML Table converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getHtmlTableToCsvDirectory();
  ...
  String _sanitizeBaseName(String input) {
    ...
    } else if (base.toLowerCase().endsWith('.htm')) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract tables from HTML files into CSV format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_html';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertHtmlToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
