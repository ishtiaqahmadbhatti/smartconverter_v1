...
class JsonObjectsToCsvFromCsvCategoryPage extends StatefulWidget {
  const JsonObjectsToCsvFromCsvCategoryPage({super.key});

  @override
  State<JsonObjectsToCsvFromCsvCategoryPage> createState() => _JsonObjectsToCsvFromCsvCategoryPageState();
}

class _JsonObjectsToCsvFromCsvCategoryPageState extends State<JsonObjectsToCsvFromCsvCategoryPage> with AdHelper {
  ...
  String _statusMessage = 'Select a JSON file containing objects to begin.';
      ...
      if (extension != '.json') {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Only JSON files are supported.'),
  ...
  Future<void> _convertJsonObjectsToCsv() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'JSON Objects to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      setState(() {
        ...
        _statusMessage = 'JSON objects converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getJsonObjectsToCsvDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert structured JSON objects into CSV rows.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
