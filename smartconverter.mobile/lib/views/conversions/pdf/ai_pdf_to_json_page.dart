...
class _AiPdfToJsonPageState extends State<AiPdfToJsonPage> with AdHelper {
  ...
  Future<void> _convertPdfToJson() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'AI PDF-to-JSON');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      final result = await _service.convertPdfToJson(
        _selectedFile!,
  ...
  Future<void> _saveJsonFile() async {
    ...
    try {
      // Use the consolidated JSONConversion/pdf-to-json folder always
      final directory = await FileManager.getJsonPdfToJsonDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract structured data from your PDF documents into clean JSON files for APIs, automation, or analysis.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertPdfToJson : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '2',
            'Tap "Convert to JSON" to extract structured data',
          ),
          const SizedBox(height: 8),
