...
class _MarkdownToPdfPageState extends State<MarkdownToPdfPage> {
  ...
  File? _selectedFile;
  MarkdownToPdfResult? _conversionResult;
  ...
  Future<void> _pickMarkdownFile() async {
    try {
      ...
      final message = 'Failed to select Markdown file: $e';
      if (mounted) {
  ...
  Future<void> _convertMarkdownToPdf() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a Markdown file first.'),
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting Markdown to PDF...';
      _conversionResult = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      final result = await _service.convertMarkdownToPdf(
        _selectedFile!,
  ...
  Future<void> _savePdfFile() async {
    ...
    try {
      final directory = await FileManager.getMarkdownToPdfDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert your Markdown files (.md, .markdown) to PDF format with proper formatting, headings, tables, and emojis.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            ...
            label: Text(
              _selectedFile == null ? 'Select Markdown File' : 'Change File',
            ),
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertMarkdownToPdf : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '1',
            'Select a Markdown file (.md or .markdown)',
          ),
          ...
          _buildInstructionStep(
            '3',
            'Save the PDF to your device or share it',
          ),
        ],
      ),
    );
