...
class _AiPdfToCsvPageState extends State<AiPdfToCsvPage> with AdHelper<AiPdfToCsvPage> {
  ...
  Future<void> _convertPdfToCsv() async {
    ...
    try {
      ...
      final result = await _service.convertPdfToCsv(
        _selectedFile!,
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final directory = await FileManager.getPdfToCsvDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract table data from PDFs into clean CSV spreadsheets ready for Excel or Google Sheets.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertPdfToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
