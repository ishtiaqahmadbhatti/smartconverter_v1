...
class _AiPdfToMarkdownPageState extends State<AiPdfToMarkdownPage> with AdHelper<AiPdfToMarkdownPage> {
  ...
  Future<void> _convertPdfToMarkdown() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting PDF to Markdown...';
      _conversionResult = null;
      _savedFilePath = null;
    });

    // Check for rewarded ad first
    final adWatched = await showRewardedAdGate(toolName: 'AI PDF-to-Markdown');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      final result = await _service.convertPdfToMarkdown(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'Markdown file converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Markdown file ready: ${result.fileName}'),
  ...
  Future<void> _shareMarkdownFile() async {
    final result = _conversionResult;
    if (result == null) return;
    ...
    if (!await fileToShare.exists()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Markdown file is not available on disk.'),
            backgroundColor: AppColors.error,
          ),
        );
      }
    ...
    ], text: 'Converted Markdown: ${result.fileName}');
  }
  ...
  String _ensureMarkdownExtension(String base) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform PDF documents into clean Markdown (.md) files with proper headings, lists, and code formatting.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        helperText: '.md extension is added automatically',
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertPdfToMarkdown : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
          ...
          Row(
            children: [
              Flexible(
                flex: isSaved ? 3 : 1,
                child: ElevatedButton.icon(
                  onPressed: _isSaving ? null : _saveMarkdownFile,
                  icon: _isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '2',
            'Tap "Convert to Markdown" to process the document',
          ),
          const SizedBox(height: 8),
          _buildInstructionStep(
            '3',
            'Save or share the generated .md file',
          ),
        ],
      ),
    );
