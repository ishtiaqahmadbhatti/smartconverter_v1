...
class PdfToTextPage extends StatefulWidget {
  const PdfToTextPage({super.key});

  @override
  State<PdfToTextPage> createState() => _PdfToTextPageState();
}

class _PdfToTextPageState extends State<PdfToTextPage> with AdHelper<PdfToTextPage> {
  ...
  Future<void> _convertPdfToText() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting PDF to Text...';
    ...
    try {
      ...
      final result = await _service.convertPdfToText(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'Text file converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Text file ready: ${result.fileName}'),
  ...
  Future<void> _saveTextFile() async {
    ...
    try {
      final directory = await FileManager.getPdfToTextDirectory();
      ...
      if (_fileNameController.text.trim().isNotEmpty) {
        ...
        targetFileName = _ensureTextExtension(customName);
      } else {
  ...
  String _ensureTextExtension(String base) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract text content from your PDF files and save it as a plain text (.txt) file.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertPdfToText : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '2',
            'Click "Convert to Text" to start conversion',
          ),
          const SizedBox(height: 8),
          _buildInstructionStep(
            '3',
            'Save the text file to your device or share it',
          ),
        ],
      ),
    );
