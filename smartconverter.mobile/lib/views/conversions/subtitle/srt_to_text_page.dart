...
class _SrtToTextPageState extends State<SrtToTextPage> with AdHelper {
  ...
  Future<void> _convertSrtToText() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'SRT to Text');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
  ...
  Future<void> _saveTextFile() async {
    ...
    try {
      final directory = await FileManager.getSrtToTextSubtitleDirectory();
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .srt to a plain .txt file',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertSrtToText : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
