...
class SrtToExcelPage extends StatefulWidget {
  const SrtToExcelPage({super.key});
  @override
  State<SrtToExcelPage> createState() => _SrtToExcelPageState();
}

class _SrtToExcelPageState extends State<SrtToExcelPage> with AdHelper {
  ...
  Future<void> _convertSrtToExcel() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'SRT to Excel');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting SRT to Excel...';
    });
    ...
    try {
      ...
      final result = await _service.convertSrtToExcel(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'Excel converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Excel ready: ${result.fileName}'),
  ...
  String _ensureXlsxExtension(String base) {
    final trimmed = base.trim();
    final lower = trimmed.toLowerCase();
    if (lower.endsWith('.xlsx')) return trimmed;
    if (lower.endsWith('.xls')) return '${trimmed}x';
    return '$trimmed.xlsx';
  }
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .srt to a structured .xlsx file',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertSrtToExcel : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
