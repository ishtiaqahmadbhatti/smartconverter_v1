...
class SrtToCsvFromSubtitlePage extends StatefulWidget {
  ...
  @override
  State<SrtToCsvFromSubtitlePage> createState() =>
      _SrtToCsvFromSubtitlePageState();
}

class _SrtToCsvFromSubtitlePageState extends State<SrtToCsvFromSubtitlePage> with AdHelper {
  ...
  Future<void> _convertSrtToCsv() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'SRT to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      final result = await _service.convertSrtToCsv(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'CSV converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('CSV ready: ${result.fileName}'),
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final directory = await FileManager.getSrtToCsvSubtitleDirectory();
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .srt to a structured .csv file',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
