...
class _ExcelToSrtPageState extends State<ExcelToSrtPage> with AdHelper {
  ...
  Future<void> _pickExcelFile() async {
    try {
      ...
      if (ext != '.xlsx' && ext != '.xls') {
        if (mounted) {
          setState(() => _statusMessage = 'Please select an Excel file (.xlsx, .xls).');
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Unsupported file. Please choose a .xlsx or .xls file.'),
      ...
      setState(() {
        ...
        _savedFilePath = null;
        _statusMessage = 'Excel selected: ${p.basename(file.path)}';
  ...
  Future<void> _convertExcelToSrt() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'Excel to SRT');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting Excel to SRT...';
    });
    ...
    try {
      ...
      final result = await _service.convertExcelToSrt(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'SRT converted successfully!';
        _savedFilePath = null;
      });
  ...
  Future<void> _saveSrtFile() async {
    ...
    try {
      final directory = await FileManager.getExcelToSrtSubtitleDirectory();
  ...
  Future<void> _shareSrtFile() async {
    ...
    ], text: 'Converted SRT: ${result.fileName}');
  }
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .xlsx/.xls to .srt format',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertExcelToSrt : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
