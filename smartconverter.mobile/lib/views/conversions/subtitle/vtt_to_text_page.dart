...
class _VttToTextPageState extends State<VttToTextPage> with AdHelper {
  ...
  Future<void> _convertVttToText() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'VTT to Text');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
  ...
  Future<void> _saveTextFile() async {
    ...
    try {
      final directory = await FileManager.getVttToTextSubtitleDirectory();
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .vtt to a plain .txt file',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertVttToText : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
