...
/// A generic page for eBook format conversions.
/// 
/// Supports optional extra form fields (e.g. for Title/Author in Markdown conversion).
class EbookCommonPage extends StatefulWidget {
  final String toolName;
  final String inputExtension;
  final String outputExtension;
  final String apiEndpoint;
  final String outputFolder; // e.g., 'markdown-to-epub'
  ...
  @override
  State<EbookCommonPage> createState() => _EbookCommonPageState();
}

class _EbookCommonPageState extends State<EbookCommonPage> {
  ...
  Future<void> _pickFile() async {
    try {
      final file = await _service.pickFile(
        allowedExtensions: [widget.inputExtension],
        type: 'custom',
      );
      ...
      // Simple check, or allow anything if needed
      if (!extension.endsWith('.${widget.inputExtension.toLowerCase()}')) {
         // Fallback check if extension might be missing dot or case issues
      }
  ...
  Future<void> _convert() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Please select a ${widget.inputExtension.toUpperCase()} file first.'),
    ...
    try {
      ...
      // Retrieve extra params if any (e.g. Title, Author)
  ...
  Future<void> _saveFile() async {
    ...
    try {
      // Use structured path: SmartConverter/EBookConversion/{outputFolder}
      final root = await FileManager.getSmartConverterDirectory();
      final ebookRoot = Directory('${root.path}/EBookConversion');
      if (!await ebookRoot.exists()) {
        await ebookRoot.create(recursive: true);
      }
      final targetDir = Directory('${ebookRoot.path}/${widget.outputFolder}');
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Extra widgets (Title/Author for markdown)
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_ebook';
