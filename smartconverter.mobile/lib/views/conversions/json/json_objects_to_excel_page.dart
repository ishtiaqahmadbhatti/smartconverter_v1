...
class _JsonObjectsToExcelPageState extends State<JsonObjectsToExcelPage> {
  ...
  Future<void> _convertJsonObjectsToExcel() async {
    ...
    try {
      ...
      final result = await _service.convertJsonObjectsToExcel(
        _selectedFile!,
  ...
  Future<void> _saveExcelFile() async {
    ...
    try {
      final directory = await FileManager.getJsonObjectsToExcelDirectory();
  ...
  String _ensureExcelExtension(String base) {
    ...
    return trimmed.toLowerCase().endsWith('.xlsx')
        ? trimmed
        : '$trimmed.xlsx';
  }
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert a list of JSON objects into Excel format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertJsonObjectsToExcel : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _buildInstructionStep('3', 'Tap "Convert to Excel" to start.'),
          const SizedBox(height: 8),
          _buildInstructionStep('4', 'Save or share the converted file.'),
        ],
      ),
    );
  }
