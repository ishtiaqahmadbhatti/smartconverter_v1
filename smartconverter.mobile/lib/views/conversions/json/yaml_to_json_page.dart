...
class _YamlToJsonPageState extends State<YamlToJsonPage> {
  ...
  Future<void> _pickYamlFile() async {
    try {
      ...
      if (extension != '.yaml' && extension != '.yml') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Please select a valid YAML file (.yaml or .yml)'),
      ...
      final message = 'Failed to select YAML file: $e';
      if (mounted) {
  ...
  Future<void> _convertYamlToJson() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a YAML file first.'),
    ...
    try {
      ...
      final result = await _service.convertYamlToJson(
        _selectedFile!,
  ...
  Future<void> _saveJsonFile() async {
    ...
    try {
      final directory = await FileManager.getYamlToJsonDirectory();
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    if (base.toLowerCase().endsWith('.yaml') || base.toLowerCase().endsWith('.yml')) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform YAML files into JSON format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickYamlFile,
            icon: const Icon(Icons.file_open_outlined),
            label: Text(
              _selectedFile == null ? 'Select YAML File' : 'Change File',
            ),
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_yaml';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertYamlToJson : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep('1', 'Select a YAML file (.yaml or .yml extension)'),
          const SizedBox(height: 8),
          _buildInstructionStep('2', 'Tap "Convert to JSON" to transform the content'),
          const SizedBox(height: 8),
