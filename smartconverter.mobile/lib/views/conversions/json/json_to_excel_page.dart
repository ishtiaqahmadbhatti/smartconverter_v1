...
class _JsonToExcelPageState extends State<JsonToExcelPage> with AdHelper<JsonToExcelPage> {
  ...
  Future<void> _convertJsonToExcel() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting JSON to Excel...';
      _conversionResult = null;
      _savedFilePath = null;
    });

    // Check for rewarded ad first
    final adWatched = await showRewardedAdGate(toolName: 'JSON to Excel');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      final result = await _service.convertJsonToExcel(
        _selectedFile!,
  ...
  Future<void> _saveExcelFile() async {
    ...
    try {
      final directory = await FileManager.getJsonJsonToExcelDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform JSON files into structured Excel format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertJsonToExcel : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
