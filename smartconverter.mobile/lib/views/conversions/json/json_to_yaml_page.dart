...
class _JsonToYamlPageState extends State<JsonToYamlPage> with AdHelper<JsonToYamlPage> {
  ...
  Future<void> _convertJsonToYaml() async {
    ...
    // Check for rewarded ad (optional for non-AI tools, but staying consistent)
    final adWatched = await showRewardedAdGate(toolName: 'JSON-to-YAML');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting JSON to YAML...';
    });
    ...
    try {
      ...
      final result = await _service.convertJsonToYaml(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'YAML file converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('YAML file ready: ${result.fileName}'),
  ...
  Future<void> _saveYamlFile() async {
  ...
  Future<void> _shareYamlFile() async {
    final result = _conversionResult;
    if (result == null) return;
    ...
    if (!await fileToShare.exists()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('YAML file is not available on disk.'),
            backgroundColor: AppColors.error,
          ),
        );
      }
    ...
    ], text: 'Converted YAML: ${result.fileName}');
  }
  ...
  String _ensureYamlExtension(String base) {
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform JSON files into YAML format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        helperText: '.yaml extension is added automatically',
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertJsonToYaml : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
          ...
          Row(
            children: [
              Flexible(
                flex: isSaved ? 3 : 1,
                child: ElevatedButton.icon(
                  onPressed: _isSaving ? null : _saveYamlFile,
                  icon: _isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstructionStep(
            '2',
            'Tap "Convert to YAML" to transform the content',
          ),
          const SizedBox(height: 8),
          _buildInstructionStep('3', 'Save or share the generated YAML file'),
        ],
      ),
    );
  }
