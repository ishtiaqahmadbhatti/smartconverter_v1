...
class PdfToPngImagePage extends StatefulWidget {
  final bool useImageCategoryStorage;

  const PdfToPngImagePage({super.key, this.useImageCategoryStorage = false});

  @override
  State<PdfToPngImagePage> createState() => _PdfToPngImagePageState();
}

class _PdfToPngImagePageState extends State<PdfToPngImagePage> {
  ...
  Future<void> _convertPdfToPng() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting PDF to PNG images...';
    });
    ...
    try {
      ...
      final result = await _service.convertPdfToPng(
        _selectedFile!,
      ...
      if (result == null || result.files.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Conversion finished, but image downloads failed. Please try again.',
            ),
            backgroundColor: AppColors.warning,
          ),
        );
      ...
      setState(() {
        ...
        final pages = result.pagesProcessed > 0
            ...
            : result.files.length;
        _statusMessage =
            'Converted $pages pages. Downloaded ${result.files.length} PNG images.';
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Downloaded ${result.files.length} PNG images (folder: ${result.folderName}).',
          ),
  ...
  Future<void> _saveImagesLocally() async {
    ...
    if (result == null || result.files.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No images to save yet.'),
          backgroundColor: AppColors.warning,
        ),
      );
      return;
    ...
    try {
      final baseDir = widget.useImageCategoryStorage
          ? await _getImageCategoryDirectory('pdf_to_png')
          : await FileManager.getPdfToPngImagesDirectory();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Images saved to: ${destination.path}'),
          backgroundColor: AppColors.success,
        ),
      );
    } catch (e) {
  ...
  Future<void> _shareImages() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        shareFiles,
        text: 'Converted PNG images for: ${result.folderName}',
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Unable to share images: $e'),
          backgroundColor: AppColors.error,
        ),
      );
    } finally {
        ...
        children: [
          ...
          Text(
            _suggestedBaseName != null
                ...
                : 'You can set a custom base name for the images folder.',
            style: const TextStyle(
              color: AppColors.textSecondary,
              fontSize: 13,
            ),
  ...
  Widget _buildInstructions() {
    final steps = [
      ...
      'Tap the convert button to export every page as PNG.',
