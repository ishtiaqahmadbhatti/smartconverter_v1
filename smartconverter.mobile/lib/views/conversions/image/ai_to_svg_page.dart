...
class _AiToSvgPageState extends State<AiToSvgPage> {
  ...
  String _statusMessage = 'Select an AI (Adobe Illustrator) file to begin.';
  ...
  Future<void> _pickAiFile() async {
    try {
      final file = await _service.pickFile(
        allowedExtensions: const ['ai'],
        type: 'custom',
      );
      ...
      if (extension != '.ai') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select an AI file.',
          );
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Only Adobe Illustrator (.ai) files are supported.',
              ),
      ...
      setState(() {
        ...
        _downloadUrl = null;
        _savedFilePath = null;
        _statusMessage = 'AI file selected: ${p.basename(file.path)}';
      });

      _updateSuggestedFileName();
    } catch (e) {
      final message = 'Failed to select AI file: $e';
      if (mounted) {
  ...
  Future<void> _convertAiToSvg() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select an AI file first.'),
          backgroundColor: AppColors.warning,
        ),
      );
      return;
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting AI to SVG...';
      _convertedFile = null;
      _downloadUrl = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      final response = await dio.post(
        ApiConfig.imageAiToSvgEndpoint,
        data: formData,
      );
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        setState(() {
          ...
          _statusMessage = 'AI converted to SVG successfully!';
        });

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('SVG file ready: $outputFilename'),
  ...
  Future<void> _saveSvgFile() async {
    ...
    try {
      // Use structured path: SmartConverter/ImageConversion/ai-to-svg
      ...
      final targetDir = Directory('${imageRoot.path}/ai-to-svg');
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    if (base.toLowerCase().endsWith('.ai')) {
  ...
  void _resetForNewConversion() {
    setState(() {
      ...
      _suggestedBaseName = null;
      _savedFilePath = null;
      _statusMessage = 'Select an AI file to begin.';
      _fileNameController.clear();
    });
    _admobService.preloadAd();
  }
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert Adobe Illustrator files to Scalable Vector Graphics.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  // ... (Other widgets reuse mostly identical code, ActionButtons, FileCard, etc)
  // I will just copy them from the previous file to ensure completeness
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickAiFile,
            icon: const Icon(Icons.file_open_outlined),
            label: Text(
              _selectedFile == null ? 'Select AI File' : 'Change File',
            ),
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_ai';
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        helperText: '.svg extension is added automatically',
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertAiToSvg : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(
                  Icons.image_aspect_ratio, // Used image_aspect_ratio instead of data_object
                  color: AppColors.textPrimary,
                  size: 24,
                ),
              ),
          ...
          Row(
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: _isSaving ? null : _saveSvgFile,
