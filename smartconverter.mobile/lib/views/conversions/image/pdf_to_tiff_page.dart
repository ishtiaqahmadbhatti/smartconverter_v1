...
class _PdfToTiffImagePageState extends State<PdfToTiffImagePage> {
  ...
  Future<void> _convertPdfToTiff() async {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting PDF to TIFF images...';
    });
    ...
    try {
      ...
      final result = await _service.convertPdfToTiff(
        _selectedFile!,
      ...
      setState(() {
        ...
        final pages = result.pagesProcessed > 0
            ...
            : result.files.length;
        _statusMessage =
            'Converted $pages pages. Downloaded ${result.files.length} TIFF images.';
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Downloaded ${result.files.length} TIFF images (folder: ${result.folderName}).',
          ),
  ...
  Future<void> _saveImagesLocally() async {
    ...
    try {
      final baseDir = widget.useImageCategoryStorage
          ? await _getImageCategoryDirectory('pdf_to_tiff')
          : await FileManager.getPdfToTiffImagesDirectory();
  ...
  Future<void> _shareImages() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        shareFiles,
        text: 'Converted TIFF images for: ${result.folderName}',
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text(
                  'Convert each page of your PDF into high-quality TIFF images.',
                  style: TextStyle(
