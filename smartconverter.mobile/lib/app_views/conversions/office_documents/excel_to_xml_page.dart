...
class _ExcelToXmlOfficePageState extends State<ExcelToXmlOfficePage> {
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (extension != '.xls' && extension != '.xlsx') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select a valid Excel file.',
          );
  ...
  Future<void> _convertFile() async {
    ...
    try {
      ...
      final formData = FormData.fromMap({
        ...
        'root_name': _rootNameController.text.trim().isEmpty ? 'data' : _rootNameController.text.trim(),
        'record_name': _recordNameController.text.trim().isEmpty ? 'record' : _recordNameController.text.trim(),
      });

      final response = await dio.post(
        ApiConfig.officeExcelToXmlEndpoint,
        data: formData,
      );
  ...
  Future<void> _saveFile() async {
    ...
    try {
      final targetDir = await FileManager.getOfficeExcelToXmlDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert Excel data to structured XML.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
