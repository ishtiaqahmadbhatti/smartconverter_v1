...
class _AiJpgToJsonPageState extends State<AiJpgToJsonPage> with AdHelper<AiJpgToJsonPage> {
  ...
  Future<void> _pickJpgFile() async {
    try {
      ...
      final message = 'Failed to select JPG file: $e';
      if (mounted) {
  ...
  Future<void> _convertJpgToJson() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a JPG file first.'),
          backgroundColor: AppColors.warning,
        ),
      );
      return;
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting JPG to JSON...';
      _convertedFile = null;
      _downloadUrl = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        setState(() {
          ...
          _statusMessage = 'JPG converted to JSON successfully!';
        });
  ...
  Future<void> _saveJsonFile() async {
    ...
    try {
      // Use structured path: SmartConverter/ImageConversion/ai-jpg-to-json
      ...
      final targetDir = Directory('${imageRoot.path}/ai-jpg-to-json');
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract structured data from JPG images using AI.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickJpgFile,
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_jpg';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertJpgToJson : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
