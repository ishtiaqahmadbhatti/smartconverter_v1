...
class _VttToSrtPageState extends State<VttToSrtPage> with AdHelper {
  ...
  Future<void> _convertVttToSrt() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'VTT to SRT');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting VTT to SRT...';
    });
    ...
    try {
      ...
      final result = await _service.convertVttToSrt(
        _selectedFile!,
  ...
  Future<void> _saveSrtFile() async {
    ...
    try {
      final directory = await FileManager.getVttToSrtSubtitleDirectory();
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .vtt to .srt format',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickVttFile,
            icon: const Icon(Icons.file_open_outlined),
            label: Text(
              _selectedFile == null ? 'Select VTT File' : 'Change File',
            ),
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertVttToSrt : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
