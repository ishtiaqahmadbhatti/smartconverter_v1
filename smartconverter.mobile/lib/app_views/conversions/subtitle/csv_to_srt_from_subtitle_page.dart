...
class _CsvToSrtFromSubtitlePageState extends State<CsvToSrtFromSubtitlePage> with AdHelper {
  ...
  Future<void> _pickCsvFile() async {
    try {
      ...
      if (ext != '.csv') {
        if (mounted) {
          setState(() => _statusMessage = 'Please select a CSV file (.csv).');
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Unsupported file. Please choose a .csv file.'),
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .csv to .srt format',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
