...
class _SrtToVttPageState extends State<SrtToVttPage> with AdHelper {
  ...
  Future<void> _convertSrtToVtt() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'SRT to VTT');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting SRT to VTT...';
    });
    ...
    try {
      ...
      final result = await _service.convertSrtToVtt(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'VTT converted successfully!';
        _savedFilePath = null;
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('VTT ready: ${result.fileName}'),
  ...
  Future<void> _saveVttFile() async {
  ...
  Future<void> _shareVttFile() async {
    final result = _conversionResult;
    if (result == null) return;
    ...
    if (!await fileToShare.exists()) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('VTT file is not available on disk.'),
            backgroundColor: AppColors.error,
          ),
        );
      }
    ...
    ], text: 'Converted VTT: ${result.fileName}');
  }
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    if (base.toLowerCase().endsWith('.vtt')) {
  ...
  String _ensureVttExtension(String base) {
    final trimmed = base.trim();
    return trimmed.toLowerCase().endsWith('.vtt') ? trimmed : '$trimmed.vtt';
  }
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 6),
                Text(
                  'Convert subtitle captions from .srt to .vtt format',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildFileNameField() {
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        helperText: '.vtt extension is added automatically',
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertSrtToVtt : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
          ...
          Row(
            children: [
              Flexible(
                flex: isSaved ? 3 : 1,
                child: ElevatedButton.icon(
                  onPressed: _isSaving ? null : _saveVttFile,
                  icon: _isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
