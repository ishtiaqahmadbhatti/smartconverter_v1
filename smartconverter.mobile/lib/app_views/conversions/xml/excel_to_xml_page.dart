...
class _ExcelToXmlPageState extends State<ExcelToXmlPage> with AdHelper<ExcelToXmlPage> {
  ...
  Future<void> _convertExcelToXml() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'Excel-to-XML');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    try {
      ...
      final result = await _service.convertExcelToXml(
        _selectedFile!,
        outputFilename: customFilename,
        rootName: _rootNameController.text,
  ...
  Future<void> _saveXmlFile() async {
    ...
    try {
      final directory = await FileManager.getExcelToXmlDirectory();
