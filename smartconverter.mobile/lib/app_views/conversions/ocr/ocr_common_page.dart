...
class OcrCommonPage extends StatefulWidget {
  ...
  final String inputExtension; // e.g., 'png', 'jpg', 'pdf'
  final String outputExtension; // e.g., 'txt', 'pdf'
  final String apiEndpoint;
  final String outputFolder; // e.g., 'png-to-text'
...
class _OcrCommonPageState extends State<OcrCommonPage> with AdHelper {
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (extension != '.${widget.inputExtension}') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select a ${widget.inputExtension.toUpperCase()} file.',
          );
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Only ${widget.inputExtension.toUpperCase()} files are supported.',
              ),
  ...
  Future<void> _convert() async {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data['success'] == true) {
        ...
        final extractedText = response.data['extracted_text'] as String?;
  ...
  Future<void> _saveFile() async {
    ...
    try {
      // Use structured path: SmartConverter/OCRConversion/{outputFolder}
      final root = await FileManager.getSmartConverterDirectory();
      final ocrRoot = Directory('${root.path}/OCRConversion');
      if (!await ocrRoot.exists()) {
        await ocrRoot.create(recursive: true);
      }
      final targetDir = Directory('${ocrRoot.path}/${widget.outputFolder}');
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    // Remove extension if present (though logic usually handles basename without extension)
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Use OCR technology to convert files.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
