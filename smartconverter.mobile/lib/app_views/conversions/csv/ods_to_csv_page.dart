...
class OdsToCsvPage extends StatefulWidget {
  const OdsToCsvPage({super.key});

  @override
  State<OdsToCsvPage> createState() => _OdsToCsvPageState();
}

class _OdsToCsvPageState extends State<OdsToCsvPage> with AdHelper {
  ...
  String _statusMessage = 'Select an ODS file to begin.';
  ...
  Future<void> _pickOdsFile() async {
    try {
      ...
      if (extension != '.ods') {
        if (mounted) {
          setState(
            () => _statusMessage = 'Please select an ODS file.',
          );
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Only ODS files are supported.'),
      ...
      setState(() {
        ...
        _savedFilePath = null;
        _statusMessage = 'ODS file selected: ${p.basename(file.path)}';
      ...
      final message = 'Failed to select ODS file: $e';
      if (mounted) {
  ...
  Future<void> _convertOdsToCsv() async {
    if (_selectedFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select an ODS file first.'),
          backgroundColor: AppColors.warning,
        ),
      );
      return;
    ...
    final adWatched = await showRewardedAdGate(toolName: 'ODS to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting ODS to CSV...';
    });
    ...
    try {
      ...
      final result = await _service.convertOdsToCsv(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'ODS converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getOdsToCsvDirectory();
  ...
  String _sanitizeBaseName(String input) {
    var base = input.trim();
    if (base.toLowerCase().endsWith('.ods')) {
  ...
  void _resetForNewConversion() {
    setState(() {
      ...
      _suggestedBaseName = null;
      _savedFilePath = null;
      _statusMessage = 'Select an ODS file to begin.';
      _fileNameController.clear();
      resetAdStatus(null);
    });
  }
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform OpenDocument Spreadsheets into CSV.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
      ...
      children: [
        Expanded(
          child: ElevatedButton.icon(
            onPressed: _isConverting ? null : _pickOdsFile,
            icon: const Icon(Icons.file_open_outlined),
            label: Text(
              _selectedFile == null ? 'Select ODS File' : 'Change File',
            ),
  ...
  Widget _buildFileNameField() {
    ...
    final hintText = _suggestedBaseName ?? 'converted_ods';
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertOdsToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
