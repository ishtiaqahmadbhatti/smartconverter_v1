...
class _CsvToExcelPageState extends State<CsvToExcelPage> with AdHelper {
  ...
  Future<void> _convertCsvToExcel() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'CSV to Excel');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting CSV to Excel...';
    });
    ...
    try {
      ...
      final result = await _service.convertCsvToExcel(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'CSV converted to Excel successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveExcelFile() async {
    ...
    try {
      final targetDir = await FileManager.getCsvToExcelDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform CSV files into Excel spreadsheets.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertCsvToExcel : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
