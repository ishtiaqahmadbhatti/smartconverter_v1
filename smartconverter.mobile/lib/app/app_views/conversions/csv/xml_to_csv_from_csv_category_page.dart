...
class XmlToCsvFromCsvCategoryPage extends StatefulWidget {
  const XmlToCsvFromCsvCategoryPage({super.key});

  @override
  State<XmlToCsvFromCsvCategoryPage> createState() => _XmlToCsvFromCsvCategoryPageState();
}

class _XmlToCsvFromCsvCategoryPageState extends State<XmlToCsvFromCsvCategoryPage> with AdHelper {
  ...
  Future<void> _convertXmlToCsv() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'XML to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting XML to CSV...';
    });
    ...
    try {
      ...
      final result = await _service.convertXmlToCsv(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'XML converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getXmlToCsvDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Transform XML data into structured CSV format.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertXmlToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
