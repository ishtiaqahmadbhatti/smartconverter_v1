...
class _ExcelToCsvFromCsvCategoryPageState extends State<ExcelToCsvFromCsvCategoryPage> with AdHelper {
      ...
      if (extension != '.xls' && extension != '.xlsx') {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Only Excel files are supported.'),
  ...
  Future<void> _convertExcelToCsv() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'Excel to CSV');
    if (!adWatched) {
      setState(() {
        _isConverting = false;
    ...
    setState(() {
      _statusMessage = 'Converting Excel to CSV...';
    });
    ...
    try {
      ...
      final result = await _service.convertExcelToCsv(
        _selectedFile!,
      ...
      setState(() {
        ...
        _statusMessage = 'Excel converted to CSV successfully!';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _saveCsvFile() async {
    ...
    try {
      final targetDir = await FileManager.getExcelToCsvDirectory();
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 6),
                Text(
                  'Extract data from Excel spreadsheets into CSV.',
                  style: TextStyle(
                    color: AppColors.textPrimary,
                    fontSize: 13,
                    height: 1.4,
  ...
  Widget _buildConvertButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: canConvert ? _convertExcelToCsv : null,
        style: ElevatedButton.styleFrom(
          backgroundColor: AppColors.primaryBlue,
          foregroundColor: AppColors.textPrimary,
