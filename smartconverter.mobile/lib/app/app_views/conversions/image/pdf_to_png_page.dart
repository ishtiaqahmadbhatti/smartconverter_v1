...
class _PdfToPngImagePageState extends State<PdfToPngImagePage> with AdHelper<PdfToPngImagePage> {
  ...
  Future<void> _convertPdfToPng() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'PDF-to-PNG');
    if (!adWatched) {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting to PNG...';
      _convertedFile = null;
      _extractedImages = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _statusMessage = 'Converted to PNG successfully!';
        });
  ...
  Future<void> _shareConvertedFile() async {
    ...
    try {
      if (_extractedImages != null && _extractedImages!.isNotEmpty) {
          ...
          await Share.shareXFiles(files, text: 'Converted PNG Images from PDF');
      } else {
          final path = _savedFilePath ?? _convertedFile!.path;
          await Share.shareXFiles([XFile(path)], text: 'Converted PNG File from PDF');
      }
    } catch (e) {
       ScaffoldMessenger.of(context).showSnackBar(
