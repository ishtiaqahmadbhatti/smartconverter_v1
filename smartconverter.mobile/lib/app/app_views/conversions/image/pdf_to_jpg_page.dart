...
class PdfToJpgImagePage extends StatefulWidget {
  final bool useImageCategoryStorage;

  const PdfToJpgImagePage({super.key, this.useImageCategoryStorage = false});

  @override
  State<PdfToJpgImagePage> createState() => _PdfToJpgImagePageState();
}

class _PdfToJpgImagePageState extends State<PdfToJpgImagePage> with AdHelper, ConversionMixin {
  ...
  void _handleFileNameChange() {
    // handled by mixin/model if needed, or local logic
  }
  ...
  Future<Directory> get saveDirectory async {
    ...
    final toolDir = Directory('${imageRoot.path}/pdf-to-jpg');
  ...
  Future<PdfToImagesResult?> performConversion(File? file, String? outputName) async {
    ...
    return await service.convertPdfToJpg(file, outputFilename: outputName);
  }
  ...
  Future<void> _saveImagesLocally() async {
    ...
    if (result == null || result.files.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No images to save yet.'), backgroundColor: AppColors.warning),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to save images: $e'), backgroundColor: AppColors.error),
      );
    } finally {
  ...
  Future<void> _shareImages() async {
    ...
    try {
      ...
      await Share.shareXFiles(
        shareFiles,
        text: 'Converted JPG images for: ${result.folderName}',
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Unable to share images: $e'), backgroundColor: AppColors.error),
      );
    } finally {
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${result.files.length} JPG images generated',
