...
class _PdfToJpgImagePageState extends State<PdfToJpgImagePage> with AdHelper<PdfToJpgImagePage> {
  ...
  Future<void> _convertPdfToJpg() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'PDF-to-JPG');
    if (!adWatched) {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting logic...';
      _convertedFile = null;
      _extractedImages = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _statusMessage = 'Converted to JPG successfully!';
        });
  ...
  Future<void> _shareConvertedFile() async {
    ...
    try {
      if (_extractedImages != null && _extractedImages!.isNotEmpty) {
          ...
          await Share.shareXFiles(files, text: 'Converted JPG Images from PDF');
      } else {
          final path = _savedFilePath ?? _convertedFile!.path;
          await Share.shareXFiles([XFile(path)], text: 'Converted JPG File from PDF');
      }
    } catch (e) {
       ScaffoldMessenger.of(context).showSnackBar(
