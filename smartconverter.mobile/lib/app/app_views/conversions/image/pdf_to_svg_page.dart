...
class _PdfToSvgImagePageState extends State<PdfToSvgImagePage> with AdHelper<PdfToSvgImagePage> {
  ...
  Future<void> _convertPdfToSvg() async {
    ...
    final adWatched = await showRewardedAdGate(toolName: 'PDF-to-SVG');
    if (!adWatched) {
    ...
    setState(() {
      _isConverting = true;
      _statusMessage = 'Converting to SVG...';
      _convertedFile = null;
      _extractedImages = null;
      _savedFilePath = null;
    });
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _statusMessage = 'Converted to SVG successfully!';
        });
  ...
  Future<void> _shareConvertedFile() async {
    ...
    try {
      if (_extractedImages != null && _extractedImages!.isNotEmpty) {
          ...
          await Share.shareXFiles(files, text: 'Converted SVG Images from PDF');
      } else {
          final path = _savedFilePath ?? _convertedFile!.path;
          await Share.shareXFiles([XFile(path)], text: 'Converted SVG File from PDF');
      }
    } catch (e) {
       ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: Column(
              ...
              children: [
                const ConversionHeaderCardWidget(
                  title: 'PDF to SVG Images', 
                  description: 'Convert PDF pages to SVG images.',
